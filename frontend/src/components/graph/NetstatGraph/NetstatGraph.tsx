...
function toCyEdge(from: string,to:string,label:string): cytoscape.ElementDefinition {
    return Object.assign({}, {
        data:
            {
...
const defineSourceAndDst = (podIPMappings: Dict,name: string,entry: NetstatInterface) => {
    if (entry.state === 'LISTEN'){
        const from = name
        const to = (entry.local.address !== undefined ? podIPMappings[entry.remote.address] ?? from : from)
        ...
        const from = podIPMappings[entry.remote.address] ?? remoteAddress === "127.0.0.1" ? name : remoteAddress
...
export const NetstatGraph = ({netstatMappings,podIPMappings}: NetstatGraphProps): JSX.Element => {
    const cyRef = useRef<cytoscape.Core>()
    const edges = netstatMappings.map(mapping => {
        const entries = mapping.entries.filter((e) => e !== undefined)
            .map( (entry,index) => {
                const {from,to,label,port} = defineSourceAndDst(podIPMappings,mapping.name,entry)
                return {
                    id: "id" + index,
                    from,
                    to,
        ...
        .reduce((acc,curr) =>{
            const idx = acc.findIndex((item) => item.to === curr.to && item.from === curr.from)
            if (idx === -1) {
                return [...acc,curr]
            }
            else if ( acc[idx].label === curr.label || acc[idx].label.includes(curr.label) ) {
            return acc
            } else {
    ...
    const nodes_with_duplicates = edges.map((entry) => [entry.data.source as string, entry.data.target as string]).flat(1)
    const nodes = Array.from(new Set(nodes_with_duplicates)).map(e => toCyNode(e,e))
    const layout = {
        name: 'grid',
        position() {
            return null
